...
Construct a command from this template:
    ...
    sudo -u "#10101" tmux new-session -d -s "SESSION_NAME" "~/.local/bin/pi --model \"MODEL\" \"PROMPT\" 2>/tmp/pi.err"

Make the following substitutions:

* Provide a `PROMPT` containing the context and instructions to the bot.
* Choose a `MODEL` that's appropriate to the task. Run `~/.local/bin/pi --list-models` to list all available models.
